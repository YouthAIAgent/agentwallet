...
class UniversalBalanceService:
    ...
    async def get_universal_balance(self, agent_id: uuid.UUID) -> Dict[str, Any]:
        ...
        agent_result = await self.session.execute(
            select(Agent).options(joinedload(Agent.wallets))
    ...
    async def get_spending_power(self, agent_id: uuid.UUID) -> Dict[str, Any]:
        ...
        for token, token_data in balance_data["token_summary"].items():
            ...
            # Apply policy limits
            token_limits = await self._apply_policy_limits(
                agent_id, token, available_amount, available_usd, policies
            )
        ...
        total_spending_power_usd = sum(
            spending["max_usd"] for spending in available_spending.values()
        )
        ...
        return [
            {
                ...
                "estimated_completion": tx.created_at + tx.estimated_completion_time if hasattr(tx, 'estimated_completion_time') else None
            }
    ...
    async def _get_locked_balances(self, agent_id: uuid.UUID) -> Dict[str, Any]:
        ...
        for row in escrow_result.all():
            ...
            if token_symbol == "SOL":
                ...
                readable_amount = Decimal(str(amount_lamports)) / Decimal('1000000')  # Assume 6 decimals for most tokens
    ...
    async def _get_spending_policies(self, agent_id: uuid.UUID) -> List[Policy]:
        ...
        result = await self.session.execute(
            ...
            .where(Agent.id == agent_id, Policy.is_active == True, Policy.type.in_(["spending_limit", "daily_limit"]))
        )
