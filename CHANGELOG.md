...
### Fixed
- **Smoke test MCP race condition** — the MCP check sometimes failed because the stdio server could exit on stdin EOF before answering `tools/list`, and the script only read the last output line. The script now keeps stdin open briefly and parses the full output for the tools response. Verified: 5/5 MCP runs expose 40 tools, and 3/3 full smoke-test runs pass from a completely fresh setup.
...
### Fixed
- **SDK README quickstart** — examples used dict access (`agent['id']`, `wallet["id"]`) but the SDK returns typed objects, so copy-pasting the quickstart crashed with `TypeError: 'Agent' object is not subscriptable`. The root README and the SDK README now use attribute access (`agent.id`, `wallet.address`, `balance.sol_balance`), verified end-to-end against a live API.
